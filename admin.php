...
session_start();
	if(isset($_POST['login'])){
		...
		$password = md5($_POST['pass']);
			...
			$sql = mysqli_query($conn, "SELECT * FROM `users` WHERE (`username` = '$user' OR `email` = '$user') AND `password` = '$password' AND `status` = 1  AND  `role`= 'admin'");
			...
			if(mysqli_num_rows($sql) != 1){
				...
				echo '<meta http-equiv="refresh" content="3; \'admin/index.php?var='.$_SESSION['id'].'\' " />';
			}
